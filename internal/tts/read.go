...
func Read(message, api string) error {
	conn, err := grpc.Dial(api, grpc.WithInsecure())

	if err != nil {
		return fmt.Errorf("could not connect to %s: %v", api, err)
	}
	defer conn.Close()
	...
	if err != nil {
		return fmt.Errorf("could not read [%s]: %v", text.Text, err)
	}
	...
	for {
		res, err := stream.Recv()
		if err != nil {
			return fmt.Errorf("could not receive data: %v", err)
		}
		if res.Index == -1 {
